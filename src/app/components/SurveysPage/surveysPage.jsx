...
const SurveysPage= () => {
    ...
    useEffect(() => {
        fetch("http://localhost:5000/getSurveys")
            .then(res => res.json())
            .then(data => {
                setSurveys(data)
                setLoading(false)
    ...
    const handleDownload = (id) => {
        fetch(`http://localhost:5000/downloadAnswers/${id}`, {
            method: "GET",
        })
            .then(res => {
            ...
            a.href = url;
            a.download = `survey_${id}_answers.xlsx`;
    ...
    return ( 
        <div className='container'>
            <div className="surveys__inner">
                ...
                <ul className={`${surveys.length === 0 ? 'surveys__list surveys__list-flex':'surveys__list'}`}>
 
                    <button onClick={()=>createSurvey()} className={`${surveys.length === 0 ? 'surveys__add surveys__add-center':'surveys__add'}`}>
                    ...
                    {loading ? (
                        ...
                        Array(7).fill(null).map((item, index) => (
                            ...
                            >
                                <div className="itemSkeleton__number"></div>
                                <div className="itemSkeleton__name"></div>
                            </div>
                        ))
                        ...
                        <div className="surveys__empty">
                            <div>
                                <p>Пока нет ни одной анкеты.</p>
                                <p>Нажмите "+" чтобы создать первую анкету.</p>
                            </div>
                        ...
                        surveys.map(survey => (
                            <ItemLi
                                className="surveys__item"
                                key={survey.id}
                            >
                                <p className="surveys__item-number">
                                    Анкета № {survey.id}
                                </p>

                                <p className="surveys__item-text">
                                    {survey.name}
                                </p>

                                <div className="surveys__item-modal">
