...
const SelectType = ({chooseType, typeQuestion, exist, questionIndex}) => {
    const questionTypes = [
    ...
    const [selectedQuestionType, setSelectedQuestionType] = useState(questionTypes[2].title);
    const [isOpen, setIsOpen] = useState(false);
    
    const currentFind = (typeQuestion)=>{
        
        let findingType = questionTypes.find((item,i)=>{
            if(item.id == typeQuestion){
                console.log(item) 
                return item  
            } 
        })
        ...
        return findingType.title
        
    }
    
    const currentIconFind = (typeQuestion)=>{
        let findingIconType = questionTypes.find((item,i)=>{
            if(item.id == typeQuestion){
                console.log(item) 
                return item  
            } 
        ...
        return findingIconType.iconSource
        
    }

    const [currentType, setCurrentType] = useState(questionTypes[2].id);
    const handleTypeSelect = (typeTitle, typeId) => {
        ...
        chooseType(questionIndex, typeId)
    };
        ...
        <div className="selectType">
            <div className="selectType__current" onClick={togglePopup}>
                <span className='selectType__current-icon'>
                    <img src={exist ? currentIconFind(typeQuestion) : currentTypeFunction(currentType)} alt="icon-select"/>
                </span>
                <p className='selectType__current-text'>{exist ? currentFind(typeQuestion) : selectedQuestionType}</p>
            </div>
